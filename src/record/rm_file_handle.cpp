...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    ...
    context->lock_mgr_->lock_shared_on_record(context->txn_, rid, fd_);//lab4
    RmPageHandle page_handle = fetch_page_handle(rid.page_no); // 1
    std::unique_ptr<RmRecord> recordptr{new RmRecord(file_hdr_.record_size, page_handle.get_slot(rid.slot_no))}; // 2

    //lab4:放入锁集
    LockDataId lock_data_id =  LockDataId{fd_,rid,LockDataType::RECORD};
    context->txn_->get_lock_set()->insert(lock_data_id);
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    return Rid{ page_id.page_no, free_slot_no };
}
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    PageId page_id = PageId{fd_, page_no};
...
RmPageHandle RmFileHandle::create_new_page_handle() {
    ...
    PageId page_id;
    page_id.fd = fd_;
    Page *page = buffer_pool_manager_->new_page(&page_id); // 1

    RmPageHandle new_page_handle = RmPageHandle( &file_hdr_, page ); // 2

    new_page_handle.page_hdr->next_free_page_no = file_hdr_.first_free_page_no;
    new_page_handle.page_hdr->num_records = 0;

    Bitmap::init(new_page_handle.bitmap, file_hdr_.bitmap_size); // 3
...
RmPageHandle RmFileHandle::create_page_handle() {
    ...
    if( file_hdr_.first_free_page_no == -1 )
        return create_new_page_handle(); // 1.1
    else
        return fetch_page_handle(file_hdr_.first_free_page_no); // 1.2
}
